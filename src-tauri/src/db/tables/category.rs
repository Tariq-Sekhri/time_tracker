...
pub async fn create_table(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    ...
    let row = sqlx::query!("SELECT COUNT(*) as count FROM category")
        ...
        .await?;
    if row.count == 0 {
        let defaults: &[(&str, i32, Option<&str>, bool, bool)] = &[
            ("Miscellaneous", 0, Some("#9c9c9c"), true, true),
            ("Hidden", 100, Some("#475569"), true, true),
            ("Browsing", 200, Some("#ff7300"), true, true),
            ("Music", 250, Some("#ec4899"), true, true),
            ("Reading", 300, Some("#a855f7"), true, true),
            ("Coding", 400, Some("#1100ff"), true, true),
            ("Gaming", 500, Some("#2eff89"), true, true),
            ("Watching", 600, Some("#fff700"), true, true),
            ("Social", 700, Some("#5662f6"), true, true),
        ];
