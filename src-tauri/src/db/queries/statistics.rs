...
pub struct CategoryStat {
    ...
    pub percentage: f64,
    pub percentage_change: Option<f64>, // Only for week view
...
) -> Result<Vec<CachedCategoryRegex>, AppError> {
    ...
    let mut regex: Vec<CachedCategoryRegex> = cat_regex
        ...
        .map(|reg| {
            let cat = category_map
                .get(&reg.cat_id)
                .ok_or_else(|| AppError::Db("Category not found".to_string()))?;

            let compiled_regex = Regex::new(&reg.regex)
                .map_err(|e| AppError::Regex(format!("Invalid regex: {}", e)))?;
...
fn get_day_start(timestamp: i64) -> i64 {
    ...
    dt.date_naive().and_hms_opt(0, 0, 0).unwrap().and_local_timezone(Local).unwrap().timestamp()
}
...
#[tauri::command]
pub async fn get_week_statistics(week_start: i64, week_end: i64) -> Result<WeekStatistics, AppError> {
    ...
    let most_active_day = day_totals.iter().max_by_key(|(_, &duration)| duration)
        .map(|(&timestamp, &duration)| (timestamp, duration));
    let most_inactive_day = day_totals.iter().min_by_key(|(_, &duration)| duration)
        .map(|(&timestamp, &duration)| (timestamp, duration));
    ...
    for stat in &mut category_stats {
        let prev_duration = prev_category_durations.get(&stat.category).copied().unwrap_or(0);
        if prev_duration > 0 {
            stat.percentage_change = Some(((stat.total_duration as f64 - prev_duration as f64) / prev_duration as f64) * 100.0);
    ...
    for app_stat in &mut top_apps {
        ...
        if prev_duration > 0 {
            app_stat.percentage_change = Some(((app_stat.total_duration as f64 - prev_duration as f64) / prev_duration as f64) * 100.0);
...
#[tauri::command]
pub async fn get_day_statistics(day_start: i64, day_end: i64) -> Result<DayStatistics, AppError> {
    let mut logs = get_logs().await?;
    let skipped_apps = get_skipped_apps().await?;
