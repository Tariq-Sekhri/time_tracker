...
fn get_foreground_app() -> Result<String, AppError> {
...
fn get_foreground_app() -> Result<String, AppError> {
    ...
    unsafe {
        let workspace_class = Class::get("NSWorkspace").ok_or_else(|| {
            AppError::Other("Failed to get NSWorkspace class".to_string())
        })?;
        
        let workspace: *mut Object = msg_send![workspace_class, sharedWorkspace];
        if workspace.is_null() {
            return Err(AppError::Other("Failed to get shared workspace. Make sure the app has accessibility permissions.".to_string()));
        }
        
        let front_app: *mut Object = msg_send![workspace, frontmostApplication];
        if front_app.is_null() {
            return Err(AppError::Other("Failed to get frontmost application. Check accessibility permissions.".to_string()));
        }
        
        let app_name: *mut Object = msg_send![front_app, localizedName];
        if app_name.is_null() {
            return Err(AppError::Other("Failed to get application name".to_string()));
        }
        
        // app_name is an NSString, get UTF8String
        let c_string: *const i8 = msg_send![app_name, UTF8String];
        if c_string.is_null() {
            return Err(AppError::Other("Failed to get UTF8 string from application name".to_string()));
        }
        
        let app_name_str = std::ffi::CStr::from_ptr(c_string)
            .to_str()
            .map_err(|e| AppError::Other(format!("Failed to convert string: {}", e)))?;
...
fn get_foreground_app() -> Result<String, AppError> {
    ...
    unsafe {
        ...
        if display.is_null() {
            return Err(AppError::Other("Failed to open X display. Make sure you're running in an X11 environment (not Wayland).".to_string()));
        }
        ...
        if focus_return == 0 {
            ...
            return Err(AppError::Other("Failed to get focused window".to_string()));
        }
        ...
        if result == 0 || name.is_null() {
            ...
            if xlib::XGetClassHint(display, focus_return, &mut class_hint) != 0 {
                if !class_hint.res_class.is_null() {
                    let class_name = std::ffi::CStr::from_ptr(class_hint.res_class as *const i8)
                        .to_str()
                        .map_err(|e| AppError::Other(format!("Failed to convert class name: {}", e)))?
                        .to_string();
            ...
            xlib::XCloseDisplay(display);
            return Err(AppError::Other("Failed to get window name or class".to_string()));
        }
        
        let window_name = std::ffi::CStr::from_ptr(name)
            .to_str()
            .map_err(|e| AppError::Other(format!("Failed to convert window name: {}", e)))?
            .to_string();
...
fn generate_log() -> Result<NewLog, AppError> {
